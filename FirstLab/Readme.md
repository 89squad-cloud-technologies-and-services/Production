...
## Задание со звёздочкой (Ещё не выполнено!)
<hr>
...
Для начала нужно создать ключи на устройствах, с которых мы будем подключаться - PC A и PC B. Делается это с помощью команды `sh-keygen`, везде прожать просто Enter (см. рисунок 13-14)
...
Далее нам нужно загрузить ключи на сервер. Делаем это с помощью команлы `ssh-copy-id name@id` и тогда мы сможем зайти на него без ввода пароля (см. рисунок 16-17)
