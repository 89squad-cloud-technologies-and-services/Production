...
|AmazonEI|Cервис, который позволяет пользователям подключать ускорение логических выводов на базе GPU к инстансам Amazon EC2. Эта служба предназначена для того, чтобы помочь пользователям оптимизировать свои приложения машинного обучения (ML) и искусственного интеллекта (AI), предоставляя им необходимые ресурсы для эффективного выполнения моделей.<br><h3>Подсистемы:</h3> <ol><li>Elastic Inference Accelerators: Это аппаратные ускорители, предоставляемые Amazon Elastic Inference. Они могут быть прикреплены к экземплярам Amazon EC2 для улучшения производительности инференса в моделях машинного обучения.</li><li>Elastic Inference Optimizer: Оптимизатор, который помогает подбирать подходящий размер аппаратного ускорителя Elastic Inference для конкретной модели машинного обучения. Это помогает управлять затратами и эффективно использовать ресурсы.</li></ol>|
...
|AWSCodePipeline|Это решение для непрерывной доставки, которое позволяет моделировать, визуализировать и автоматизировать этапы развертывания программного обеспечения. Различные этапы процесса выпуска программного обеспечения можно легко смоделировать и настроить.<br><h3>Подсистемы:</h3><ol><li>Pipeline: Это центральный элемент CodePipeline. Pipeline представляет собой набор этапов и действий, которые определяют последовательность и автоматизацию процесса непрерывной поставки. Он объединяет и управляет всеми этапами доставки вашего приложения.</li><li>Action: Действия - это индивидуальные шаги в рамках конкретного этапа. Они представляют собой автоматизированные задачи, такие как сборка кода, запуск тестов, развертывание приложения и другие.</li></ol>|
...
|AmazonPersonalize|Это сервис машинного обучения (ML) с минимумом программирования, который может предоставлять отличительные предложения для любого приложения, работающего в инфраструктуре Amazon Web Services (AWS), с помощью вызова API. Цель Amazon Personalize – повысить вовлеченность пользователей, предлагая индивидуальные предложения.Amazon Personalize Service: Основной сервис, предоставляющий API для интеграции с вашими данными и создания моделей машинного обучения для персонализации рекомендаций.<br><h3>Подсистемы:</h3><ol><li>Amazon Personalize Data S3 Bucket: S3 бакет, в котором хранятся данные, необходимые для обучения модели, такие как данные о взаимодействии пользователя с контентом.</li><li>Amazon Personalize Console: Веб-интерфейс для управления и настройки процесса создания персонализированных моделей без необходимости программирования.</li><li>Amazon Personalize Runtime: Сервис, предоставляющий API для интеграции персонализированных рекомендаций в ваше приложение в реальном времени</li></ol>|
